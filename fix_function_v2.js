    ...
    BEGIN
        ...
        SELECT 
            JSON_BUILD_OBJECT(
                ...
                'flayers', (
                    SELECT CASE WHEN COUNT(*) > 0 THEN JSON_AGG(
                        JSON_BUILD_OBJECT(
                ...
                'features', (
                    SELECT CASE WHEN COUNT(*) > 0 THEN JSON_AGG(feature_obj) ELSE '[]'::JSON END
                    FROM (
                        SELECT DISTINCT 
                            JSON_BUILD_OBJECT(
                        ...
                        WHERE pb.product_id = f.product_id 
                            AND f.deleted_at IS NULL
                        ORDER BY f.no_order NULLS LAST
                    ) AS features_with_order
                ),
                'galleries', (
                    SELECT CASE WHEN COUNT(*) > 0 THEN JSON_AGG(
                        JSON_BUILD_OBJECT(
                ...
                'product_360', (
                    SELECT JSON_BUILD_OBJECT(
                        'interior', (
                            SELECT CASE WHEN COUNT(*) > 0 THEN JSON_OBJECT_AGG(sub_type_name, product_360_image) ELSE '{}'::JSON END
                        ...
                        'exterior', (
                            SELECT CASE WHEN COUNT(*) > 0 THEN JSON_AGG(JSON_BUILD_OBJECT('product_360_id', product_360_id, 'product_360_image', product_360_image)) ELSE '[]'::JSON END
                            FROM mst_360_product
                            WHERE product_id = pb.product_id
                        ...
                        'default', (
                            SELECT CASE WHEN COUNT(*) > 0 THEN JSON_AGG(product_360_image) ELSE '[]'::JSON END
                            FROM mst_360_product
                            WHERE product_id = pb.product_id
...
pgCore.raw(sql)
  .then(() => {
    console.log('✅ Function berhasil di-update!');
    return pgCore.raw("SELECT get_product_detail_json('dump-truck', 'ms600') as data");
  })
  .then((result) => {
    console.log('\n📊 Hasil Query:');
