...
const TABLE = 'mst_degree'

const COLUMN = [
  `${TABLE}.degree_id`, `${TABLE}.degree_name`, `${TABLE}.degree_description`,
...
const condition = (builder, where, search = null) => {
  ...
  if (where.degree_id) {
    builder.where(`${TABLE}.degree_id`, where.degree_id)
  }

  if (search) {
    builder.whereILike(`${TABLE}.degree_name`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
    builder.orWhereILike(`${TABLE}.degree_description`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
  }
