...
const COLUMN_ALL = [
  `${TABLE}.specification_id`, `${TABLE}.specification_name`,
...
const getByParam = async (where, column = COLUMN_ALL) => {
  try {
    const [rows] = await sql(null).clone()
      .select(column)
      .where(`${TABLE}.${PRIMARY_KEY.SPECIFICATION_PRODUCT}`, where?.[PRIMARY_KEY.SPECIFICATION_PRODUCT])
    if (rows) {
      return mappingSuccess(lang.__('get.success'), rows)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.[PRIMARY_KEY.SPECIFICATION_PRODUCT] }), rows)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
...
const update = async (where, payload, name = '') => {
  try {
    ...
    if (payload.type_method === 'update') {
      message = lang.__('updated.success', { id: where?.[PRIMARY_KEY.SPECIFICATION_PRODUCT] })
      result = await Repo.updated(TABLE, where, payload, COLUMN[0], name)
    } else {
      const format = todayFormat('YYYYMMDDhmmss')
      message = lang.__('archive.success', { id: where?.[PRIMARY_KEY.SPECIFICATION_PRODUCT] })
      const [rows] = await pgCore(TABLE).select(['specification_name']).where(where)
      if (rows) {
        payload.specification_name = `archived-${format}-${rows.specification_name}`
      }
    }
    ...
    result = await pgCore(TABLE)
      .where(where)
      .update(payload)
      .returning([PRIMARY_KEY.SPECIFICATION_PRODUCT])
    if (result) {
      return mappingSuccess(message, result)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.[PRIMARY_KEY.SPECIFICATION_PRODUCT] }), result)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
