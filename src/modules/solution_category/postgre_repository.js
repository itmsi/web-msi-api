...
const COLUMN_ALL = [
  `${TABLE}.solution_category_id`, `${TABLE}.solution_category_name_id`, `${TABLE}.solution_category_name_en`, `${TABLE}.solution_category_name_cn`,
...
const getByParam = async (where, column = COLUMN_ALL) => {
  try {
    const [rows] = await sql(null).clone()
      .select(column)
      .where(`${TABLE}.${PRIMARY_KEY.SOLUTION_CATEGORY}`, where?.[PRIMARY_KEY.SOLUTION_CATEGORY])
    if (rows) {
      return mappingSuccess(lang.__('get.success'), rows)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.solution_category_id }), rows)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
...
const update = async (where, payload, name = '') => {
  try {
    ...
    if (payload.type_method === 'update') {
      message = lang.__('updated.success', { id: where?.solution_category_id })
      result = await Repo.updated(TABLE, where, payload, COLUMN[0], name)
    } else {
      const format = todayFormat('YYYYMMDDhmmss')
      message = lang.__('archive.success', { id: where?.solution_category_id })
      const [rows] = await pgCore(TABLE).select(['solution_category_name_id', 'solution_category_name_en', 'solution_category_name_cn', 'solution_category_description']).where(where)
      if (rows) {
        payload.solution_category_name_id = `archived-${format}-${rows.solution_category_name_id}`
        payload.solution_category_name_en = `archived-${format}-${rows.solution_category_name_en}`
        payload.solution_category_name_cn = `archived-${format}-${rows.solution_category_name_cn}`
        payload.solution_category_description = `archived-${format}-${rows.solution_category_description}`
      }
    }
