...
const getByParam = async (where, column = COLUMN_ALL) => {
  try {
    const [rows] = await sql(null).clone()
      .select(column)
      .where(`${TABLE}.${PRIMARY_KEY.JOB_CAREER}`, where?.[PRIMARY_KEY.JOB_CAREER])
    if (rows) {
      return mappingSuccess(lang.__('get.success'), rows)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.job_career_id }), rows)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
...
const update = async (where, payload, name = '') => {
  try {
    ...
    if (payload.type_method === 'update') {
      message = lang.__('updated.success', { id: where?.job_career_id })
      result = await Repo.updated(TABLE, where, payload, COLUMN[0], name)
    } else {
      const format = todayFormat('YYYYMMDDhmmss')
      message = lang.__('archive.success', { id: where?.job_career_id })
      const [rows] = await pgCore(TABLE).select(['job_career_name', 'job_career_description']).where(where)
      if (rows) {
        payload.job_career_name = `archived-${format}-${rows.job_career_name}`
        payload.job_career_description = `archived-${format}-${rows.job_career_description}`
      }
    }
    delete payload?.type_method
    result = await pgCore(TABLE).where(where).update(payload).returning(['job_career_id'])
    if (result) {
      return mappingSuccess(message, result)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.job_career_id }), result)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
