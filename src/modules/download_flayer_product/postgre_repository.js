...
const TABLE = 'data_download_flayer_produk'
const COLUMN_ALL = [
  `${TABLE}.data_download_flayer_produk_id`, `${TABLE}.company_name`, `${TABLE}.email`, `${TABLE}.phone`,
  `${TABLE}.approve`,
...
const condition = (builder, where, search = null) => {
  ...
  if (where.data_download_flayer_produk_id) {
    builder.where(`${TABLE}.data_download_flayer_produk_id`, where.data_download_flayer_produk_id)
  }

  if (search) {
    builder.whereILike(`${TABLE}.company_name`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
    builder.orWhereILike(`${TABLE}.email`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
    builder.orWhereILike(`${TABLE}.phone`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
  }
...
const getByParam = async (where, column = COLUMN_ALL) => {
  try {
    const [rows] = await sql(null).clone()
      .select(column)
      .where(`${TABLE}.${PRIMARY_KEY.DATA_DOWNLOAD_FLYER_PRODUK}`, where?.[PRIMARY_KEY.DATA_DOWNLOAD_FLYER_PRODUK])
    if (rows) {
      return mappingSuccess(lang.__('get.success'), rows)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.[PRIMARY_KEY.DATA_DOWNLOAD_FLYER_PRODUK] }), rows)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
...
const update = async (where, payload, name = '') => {
  try {
    ...
    if (payload.type_method === 'update') {
      message = lang.__('updated.success', { id: where?.[PRIMARY_KEY.DATA_DOWNLOAD_FLYER_PRODUK] })
      result = await Repo.updated(TABLE, where, payload, COLUMN[0], name)
    } else {
      const format = todayFormat('YYYYMMDDhmmss')
      message = lang.__('archive.success', { id: where?.[PRIMARY_KEY.DATA_DOWNLOAD_FLYER_PRODUK] })
      const [rows] = await pgCore(TABLE).select(['company_name', 'email', 'phone', 'approve']).where(where)
      if (rows) {
        payload.company_name = `archived-${format}-${rows.company_name}`
        payload.email = `archived-${format}-${rows.email}`
        payload.phone = `archived-${format}-${rows.phone}`
        payload.approve = `archived-${format}-${rows.approve}`
      }
    }
    delete payload?.type_method
    result = await pgCore(TABLE).where(where).update(payload).returning(['data_download_flayer_produk_id'])
    if (result) {
      return mappingSuccess(message, result)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.[PRIMARY_KEY.DATA_DOWNLOAD_FLYER_PRODUK] }), result)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
