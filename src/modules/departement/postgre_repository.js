...
const TABLE = 'mst_departement'
const COLUMN_ALL = [
  `${TABLE}.departement_id`, `${TABLE}.departement_name`, `${TABLE}.departement_description`,
...
const condition = (builder, where, search = null) => {
  ...
  if (search) {
    builder.whereILike(`${TABLE}.departement_name`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
    builder.orWhereILike(`${TABLE}.departement_description`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
  }
...
const getByParam = async (where, column = COLUMN_ALL) => {
  try {
    const [rows] = await sql(null).clone()
      .select(column)
      .where(`${TABLE}.${PRIMARY_KEY.DEPARTEMENT}`, where?.[PRIMARY_KEY.DEPARTEMENT])
    if (rows) {
      return mappingSuccess(lang.__('get.success'), rows)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.departement_id }), rows)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
...
const update = async (where, payload, name = '') => {
  try {
    ...
    if (payload.type_method === 'update') {
      message = lang.__('updated.success', { id: where?.departement_id })
      result = await Repo.updated(TABLE, where, payload, COLUMN[0], name)
    } else {
      const format = todayFormat('YYYYMMDDhmmss')
      message = lang.__('archive.success', { id: where?.departement_id })
      const [rows] = await pgCore(TABLE).select(['departement_name', 'departement_description']).where(where)
      if (rows) {
        payload.departement_name = `archived-${format}-${rows.departement_name}`
        payload.departement_description = `archived-${format}-${rows.departement_description}`
      }
    }
    delete payload?.type_method
    result = await pgCore(TABLE).where(where).update(payload).returning(['departement_id'])
    if (result) {
      return mappingSuccess(message, result)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.departement_id }), result)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
