...
const TABLE = 'mst_marital_status'
const COLUMN_ALL = [
  `${TABLE}.marital_status_id`, `${TABLE}.marital_status_name`, `${TABLE}.marital_status_description`,
...
const condition = (builder, where, search = null) => {
  ...
  if (where.marital_status_id) {
    builder.where(`${TABLE}.marital_status_id`, where.marital_status_id)
  }

  if (search) {
    builder.whereILike(`${TABLE}.marital_status_name`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
    builder.orWhereILike(`${TABLE}.marital_status_description`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
  }
...
const getByParam = async (where, column = COLUMN_ALL) => {
  try {
    const [rows] = await sql(null).clone()
      .select(column)
      .where(`${TABLE}.${PRIMARY_KEY.MARITAL_STATUS}`, where?.[PRIMARY_KEY.MARITAL_STATUS])
    if (rows) {
      return mappingSuccess(lang.__('get.success'), rows)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.marital_status_id }), rows)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
...
const update = async (where, payload, name = '') => {
  try {
    ...
    if (payload.type_method === 'update') {
      message = lang.__('updated.success', { id: where?.marital_status_id })
      result = await Repo.updated(TABLE, where, payload, COLUMN[0], name)
    } else {
      const format = todayFormat('YYYYMMDDhmmss')
      message = lang.__('archive.success', { id: where?.marital_status_id })
      const [rows] = await pgCore(TABLE).select(['marital_status_name', 'marital_status_description']).where(where)
      if (rows) {
        payload.marital_status_name = `archived-${format}-${rows.marital_status_name}`
        payload.marital_status_description = `archived-${format}-${rows.marital_status_description}`
      }
    }
    delete payload?.type_method
    result = await pgCore(TABLE).where(where).update(payload).returning(['marital_status_id'])
    if (result) {
      return mappingSuccess(message, result)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.marital_status_id }), result)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
