...
const TABLE = 'mst_religion'
const COLUMN_ALL = [
  `${TABLE}.religion_id`, `${TABLE}.religion_name`, `${TABLE}.religion_description`,
...
const condition = (builder, where, search = null) => {
  ...
  if (where.religion_id) {
    builder.where(`${TABLE}.religion_id`, where.religion_id)
  }

  if (search) {
    builder.whereILike(`${TABLE}.religion_name`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
    builder.orWhereILike(`${TABLE}.religion_description`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
  }
...
const getByParam = async (where, column = COLUMN_ALL) => {
  try {
    const [rows] = await sql(null).clone()
      .select(column)
      .where(`${TABLE}.${PRIMARY_KEY.RELIGION}`, where?.[PRIMARY_KEY.RELIGION])
    if (rows) {
      return mappingSuccess(lang.__('get.success'), rows)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.[PRIMARY_KEY.RELIGION] }), rows)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
...
const update = async (where, payload, name = '') => {
  try {
    ...
    if (payload.type_method === 'update') {
      message = lang.__('updated.success', { id: where?.[PRIMARY_KEY.RELIGION] })
      result = await Repo.updated(TABLE, where, payload, COLUMN[0], name)
    } else {
      const format = todayFormat('YYYYMMDDhmmss')
      message = lang.__('archive.success', { id: where?.[PRIMARY_KEY.RELIGION] })
      const [rows] = await pgCore(TABLE).select(['religion_name', 'religion_description']).where(where)
      if (rows) {
        payload.religion_name = `archived-${format}-${rows.religion_name}`
        payload.religion_description = `archived-${format}-${rows.religion_description}`
      }
    }
    delete payload?.type_method
    result = await pgCore(TABLE).where(where).update(payload).returning(['religion_id'])
    if (result) {
      return mappingSuccess(message, result)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.[PRIMARY_KEY.RELIGION] }), result)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
