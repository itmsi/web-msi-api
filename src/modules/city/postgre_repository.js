...
const TABLE = 'mst_city'
const COLUMN_ALL = [
  `${TABLE}.city_id`, `${TABLE}.city_name`, `${TABLE}.city_description`,
...
const condition = (builder, where, search = null) => {
  ...
  if (where.city_id) {
    builder.where(`${TABLE}.city_id`, where.city_id)
  }

  if (search) {
    builder.whereILike(`${TABLE}.city_name`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
    builder.orWhereILike(`${TABLE}.city_description`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
  }
...
const getByParam = async (where, column = COLUMN_ALL) => {
  try {
    const [rows] = await sql(null).clone()
      .select(column)
      .where(`${TABLE}.${PRIMARY_KEY.CITY}`, where?.[PRIMARY_KEY.CITY])
    if (rows) {
      return mappingSuccess(lang.__('get.success'), rows)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.[PRIMARY_KEY.CITY] }), rows)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
...
const update = async (where, payload, name = '') => {
  try {
    ...
    if (payload.type_method === 'update') {
      message = lang.__('updated.success', { id: where?.[PRIMARY_KEY.CITY] })
      result = await Repo.updated(TABLE, where, payload, COLUMN[0], name)
    } else {
      const format = todayFormat('YYYYMMDDhmmss')
      message = lang.__('archive.success', { id: where?.[PRIMARY_KEY.CITY] })
      const [rows] = await pgCore(TABLE).select(['city_name', 'city_description']).where(where)
      if (rows) {
        payload.city_name = `archived-${format}-${rows.city_name}`
        payload.city_description = `archived-${format}-${rows.city_description}`
      }
    }
    delete payload?.type_method
    result = await pgCore(TABLE).where(where).update(payload).returning(['city_id'])
    if (result) {
      return mappingSuccess(message, result)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.[PRIMARY_KEY.CITY] }), result)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
