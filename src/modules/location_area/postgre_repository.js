...
const getByParam = async (where, column = COLUMN_ALL) => {
  try {
    const [rows] = await sql(null).clone()
      .select(column)
      .where(`${TABLE}.${PRIMARY_KEY.LOCATION_AREA}`, where?.[PRIMARY_KEY.LOCATION_AREA])
    if (rows) {
      return mappingSuccess(lang.__('get.success'), rows)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.location_area_id }), rows)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
...
const update = async (where, payload, name = '') => {
  try {
    ...
    if (payload.type_method === 'update') {
      message = lang.__('updated.success', { id: where?.location_area_id })
      result = await Repo.updated(TABLE, where, payload, COLUMN[0], name)
    } else {
      const format = todayFormat('YYYYMMDDhmmss')
      message = lang.__('archive.success', { id: where?.location_area_id })
      const [rows] = await pgCore(TABLE).select(['location_area_name', 'location_area_description']).where(where)
      if (rows) {
        payload.location_area_name = `archived-${format}-${rows.location_area_name}`
        payload.location_area_description = `archived-${format}-${rows.location_area_description}`
      }
    }
    delete payload?.type_method
    result = await pgCore(TABLE).where(where).update(payload).returning(['location_area_id'])
    if (result) {
      return mappingSuccess(message, result)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.location_area_id }), result)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
