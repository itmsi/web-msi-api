...
const TABLE = 'mst_feature_child_product'
const PARENT_TABLE = 'mst_feature_product'
...
const condition = (builder, where, search = null) => {
  ...
  if (where.feature_product_id) {
    builder.where(`${TABLE}.feature_product_id`, where.feature_product_id)
  }

  if (search) {
    builder.whereILike(`${TABLE}.feature_child_product_title_id`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
    builder.orWhereILike(`${TABLE}.feature_child_product_title_en`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
    builder.orWhereILike(`${TABLE}.feature_child_product_title_cn`, `%${search}%`).andWhere(`${TABLE}.deleted_at`, null)
  }
...
const sql = (where, search = false) => {
  let query = pgCore(TABLE)
    .leftJoin(PARENT_TABLE, `${TABLE}.feature_product_id`, `${PARENT_TABLE}.feature_product_id`)
...
const getByParam = async (where, column = COLUMN_ALL) => {
  try {
    const [rows] = await sql(null).clone()
      .select(column)
      .where(`${TABLE}.${PRIMARY_KEY.FEATURE_CHILD_PRODUCT}`, where?.[PRIMARY_KEY.FEATURE_CHILD_PRODUCT])
    if (rows) {
      return mappingSuccess(lang.__('get.success'), rows)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.[PRIMARY_KEY.FEATURE_CHILD_PRODUCT] }), rows)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
...
const update = async (where, payload, name = '') => {
  try {
    ...
    if (payload.type_method === 'update') {
      message = lang.__('updated.success', { id: where?.[PRIMARY_KEY.FEATURE_CHILD_PRODUCT] })
      result = await Repo.updated(TABLE, where, payload, COLUMN[0], name)
    } else {
      const format = todayFormat('YYYYMMDDhmmss')
      message = lang.__('archive.success', { id: where?.[PRIMARY_KEY.FEATURE_CHILD_PRODUCT] })
      const [rows] = await pgCore(TABLE).select(['feature_child_product_title_id', 'feature_child_product_title_en', 'feature_child_product_title_cn']).where(where)
      if (rows) {
        payload.feature_child_product_title_id = `archived-${format}-${rows.feature_child_product_title_id}`
        payload.feature_child_product_title_en = `archived-${format}-${rows.feature_child_product_title_en}`
        payload.feature_child_product_title_cn = `archived-${format}-${rows.feature_child_product_title_cn}`
      }
    }
    ...
    result = await pgCore(TABLE)
      .where(where)
      .update(payload)
      .returning([PRIMARY_KEY.FEATURE_CHILD_PRODUCT])
    if (result) {
      return mappingSuccess(message, result)
    }
    return mappingSuccess(lang.__('not.found.id', { id: where?.[PRIMARY_KEY.FEATURE_CHILD_PRODUCT] }), result)
  } catch (error) {
    error.path = __filename
    return mappingError(error)
  }
