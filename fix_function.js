...
const sql = `
    ...
    BEGIN
        ...
        SELECT 
            (
                SELECT JSON_AGG(feature_obj ORDER BY (feature_obj->>'no_order')::int NULLS LAST)
                FROM (
        ...
        SELECT 
            JSON_BUILD_OBJECT(
                'interior', (
                    SELECT JSON_OBJECT_AGG(sub_type_name, product_360_image)
                ...
                'exterior', (
                    SELECT JSON_AGG(JSON_BUILD_OBJECT('product_360_id', product_360_id, 'product_360_image', product_360_image))
                    FROM mst_360_product
                    WHERE product_id = pb.product_id
                ...
                'default', (
                    SELECT JSON_AGG(product_360_image)
                    FROM mst_360_product
                    WHERE product_id = pb.product_id
        ...
        SELECT 
            JSON_BUILD_OBJECT(
                ...
                'flayers', COALESCE(v_flayers, '[]'::json),
                'features', COALESCE(v_features, '[]'::json),
                'galleries', COALESCE(v_galleries, '[]'::json),
                'product_360', COALESCE(v_product_360, '{}'::json)
            )
        INTO result
        FROM product_base pb;
