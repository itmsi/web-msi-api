...
```
## NOTES
if install in staging or production server change<br/>
NODE_ENV to "production/staging" <br/>
in .env file<br/>
...
├── src/                * all source code in here
  ...
  └── debug/            * all debug file here
  |  └── *.js           * for debug app
  └── job/              * all job file here
  |  └── *.js           * for job app
  ...
  |   └── index.js      * test apps
  └── static            * all swagger file here open api
  |   └── path          * path swagger
  |   └── schema        * schema swagger
  └── utils/            * all utils file here
  └── views/            * all template engine html register here
```
### 1. Coverage
